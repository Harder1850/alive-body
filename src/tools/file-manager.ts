...
const ALIVE_WEB_DIR = join('C:', 'Users', 'mikeh', 'dev', 'ALIVE', 'alive-repos', 'alive-web');
 ...
 */
export function writeWebFile(filename: string, content: string): FileWriteResult {
  ...
  try {
    mkdirSync(ALIVE_WEB_DIR, { recursive: true });
    const fullPath = join(ALIVE_WEB_DIR, safe);
