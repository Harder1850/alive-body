 ...
 *   - STG decisions written verbatim as passed by runtime.
...
export function logSignalReceived(
  ...
  appendToExperienceStream({
    kind: 'signal_received', timestamp: Date.now(), signal_id: signalId, data: signalData,
  });
}
...
export function logStgDecision(
  ...
  appendToExperienceStream({
    kind: 'stg_decision', timestamp: Date.now(), signal_id: signalId,
    data: { verdict, reason, ...context },
  });
}
...
export function logActionDispatched(
  ...
  appendToExperienceStream({
    kind: 'action_dispatched', timestamp: Date.now(), signal_id: signalId,
    data: { decision_id: decisionId, action_type: actionType, payload },
  });
}
...
export function logActionOutcome(
  ...
  appendToExperienceStream({
    kind: 'action_outcome', timestamp: Date.now(), signal_id: signalId,
    data: { decision_id: decisionId, success, detail },
  });
}
...
export function logCycleComplete(
  ...
  appendToExperienceStream({
    kind: 'cycle_complete', timestamp: Date.now(), signal_id: signalId, data: record,
  });
}
...
export interface ExecutionLogEntry {
  timestamp: number; signalId: string; decisionId: string;
  actionType: string; result: string;
}
export function recordExecution(entry: ExecutionLogEntry): void {
  logActionDispatched(entry.signalId, entry.decisionId, entry.actionType);
}
export function getExecutionLog(): ExecutionLogEntry[] { return []; }
export function getLog(): ExecutionLogEntry[] { return []; }
