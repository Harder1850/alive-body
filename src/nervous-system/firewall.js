...
exports.firewallCheck = firewallCheck;
const BLOCK_RULES = [
    // §14.1 — Destructive system commands
    {
        id: 'FW-01',
...
function validateStructure(signal) {
 ...
 */
function firewallCheck(signal) {
    const result = inspect(signal);
    if (result.blocked) {
        console.warn(`[FIREWALL] BLOCKED signal ${signal.id} (source=${signal.source}): ` +
